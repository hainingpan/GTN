...
class Braiding:
    ...
    def shift_DW_ext(self, leg, start, end, time=10, drop_0=None, drop_1=None, special=False):
        '''
        This is the shift "accross leg"
        time: total time step of shifting of domain wall from `start` to `end`
        leg: which leg is operate
        start: [left, right] the position of left and right domain wall in the starting stage
        end: [left, right] the position of left and right domain wall in the final stage
        ...
        for intermediate in intermediate_pos:
            ...
            p_list.append(
                interpolation(x1=intermediate[0],
                              ...
                              L=2*self.L - 1,
                              k=self.k)[drop_1[0] + 1:drop_1[1]])
        site_0_list = self.site_0_list[leg[0]] + self.site_0_list[leg[1]][::-1]
        if not special:
            site_1_list = self.site_1_list[leg[0]] + [[self.site_0_list[leg[0]][-1][1],self.site_0_list[leg[1]][-1][1]]] + self.site_1_list[leg[1]][::-1]
        else:
            site_1_list = self.site_1_list[leg[0]] + [[self.site_0_list[leg[0]][-1][1],self.site_0_list[2][-1][1]]] + self.site_1_list[leg[1]][::-1]
        site_list = [
            site_0_list[drop_0[0] + 1:drop_0[1]],
            site_1_list[drop_1[0] + 1:drop_1[1]]
        ] * time
